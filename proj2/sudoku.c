...
* 2025-04-15 컴퓨터학부 2021044720 신동빈 함수작성
 */
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
...
{
    for (int i = 0; i < 9; i++) {
        bool check[10] = {false}; //check[1] ~ check[9] 사용
        bool is_valid = true;

        for (int j = 0; j<9; j++){
            int num = sudoku[i][j];

            //숫자가 1~9 범위를 벗어나거나, 이미 나왔던 숫자면 잘못된 행
...
void check_sudoku(void)
{
    ...
    for (int i = 0; i < 9; ++i)
    {
        int *arg = malloc(sizeof(int)); //heap 메모리 동적할당
        *arg = i;
        pthread_create(&tid[i], NULL, check_subgrid, arg);
    }
    /*
